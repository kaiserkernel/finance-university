      ...
      <TableRow hover tabIndex={-1} role="checkbox" selected={selected}>
        {headList.map((headItem: any, i) => (
          <TableCell
            ...
            align={rowKeys.includes(headItem.id) ? "center" : "left"}
          >
            <>
              {headItem.id === "announcement" ? (
                ...
                checkStatus(row[headItem.id], "reviewed") ? (
                  ...
                  <Iconify
                    ...
                    sx={{ color: "error.main" }}
                  />
                ) : headItem.id.includes("reviewer") &&
                  row[headItem.id]?.user?.email ? (
                  user.role === "col_dean" ? (
                    <Tooltip title={row[headItem.id]?.user.email}>
                      <Typography>
                        {`${row[headItem.id].user.firstName} ${
                          row[headItem.id].user.lastName
                        }`.trim()}
                      </Typography>
                    </Tooltip>
                  ) : (
                ...
                <Button
                  variant="text"
                  color={row["reviewed"] === "reviewed" ? "warning" : "primary"}
                  onClick={(e) => openApplicationDoc(e)}
                >
                  View
                </Button>
              ) : (headItem.id === "name" || headItem.id == "enrollment") &&
                user.role === "reviewer" ? (
                "----"
              ) : (
                row[headItem.id]
        ...
        >
          <DialogTitle sx={{ m: 0, p: 2 }}>Application</DialogTitle>
          <IconButton
            aria-label="close"
            onClick={handleClose}
            sx={(theme) => ({
          ...
          <DialogContent>
            ...
            >
              {user?.role != "user" &&
                (row[user?.role] == "pending" ||
                  prevState === "reviewed" ||
                  row.reviewer_1.status == "pending" ||
                  row.reviewer_2.status == "pending") && (
                  <>
                    <Button
              ...
              {/* {(user?.role == "col_dean" || user?.role == "user" || user?.role == "grant_dep" && row["grant_dir"] == "approved") && ( */}
              {
                ...
                ["col_dean", "grant_dep", "grant_dir", "finance"].includes(
                  user.role
                ) && (
                  <Button
        ...
        <Dialog open={openAskInfo} onClose={() => setOpenAskInfo(false)}>
          ...
          <Typography p={2}>Ask more information for applicant</Typography>
          <DialogActions>
